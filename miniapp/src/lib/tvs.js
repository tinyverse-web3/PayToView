...
`);
        return I != -1 && (console.log(M.substring(0, I)), M = M.substring(I + 1)), B.length;
      },
      write(C, B, I, y, k, c) {
  ...
  const D = new TextEncoder("utf-8"), U = new TextDecoder("utf-8");
  globalThis.Go = class {
    constructor() {
      ...
      const M = (A, Q) => {
        this.mem.setUint32(A + 0, Q, !0), this.mem.setUint32(A + 4, Math.floor(Q / 4294967296), !0);
      }, C = (A) => {
        const Q = this.mem.getUint32(A + 0, !0), g = this.mem.getInt32(A + 4, !0);
        ...
        w === void 0 && (w = this._idPool.pop(), w === void 0 && (w = this._values.length), this._values[w] = Q, this._goRefCounts[w] = 0, this._ids.set(Q, w)), this._goRefCounts[w]++;
        let K = 0;
        switch (typeof Q) {
          case "object":
            Q !== null && (K = 1);
        ...
        this.mem.setUint32(A + 4, 2146959360 | K, !0), this.mem.setUint32(A, w, !0);
      }, y = (A) => {
        const Q = C(A + 0), g = C(A + 8);
        return new Uint8Array(this._inst.exports.mem.buffer, Q, g);
      }, k = (A) => {
        const Q = C(A + 0), g = C(A + 8), w = new Array(g);
        for (let K = 0; K < g; K++)
          w[K] = B(Q + K * 8);
        return w;
      }, c = (A) => {
      ...
      this.importObject = {
        go: {
          ...
          "runtime.wasmExit": (A) => {
            ...
            this.exited = !0, delete this._inst, delete this._values, delete this._goRefCounts, delete this._ids, delete this._idPool, this.exit(Q);
          },
          // func wasmWrite(fd uintptr, p unsafe.Pointer, n int32)
          "runtime.wasmWrite": (A) => {
            A >>>= 0;
            const Q = C(A + 8), g = C(A + 16), w = this.mem.getInt32(A + 24, !0);
            fs.writeSync(Q, new Uint8Array(this._inst.exports.mem.buffer, g, w));
          },
          ...
          "runtime.scheduleTimeoutEvent": (A) => {
            ...
            this._nextCallbackTimeoutID++, this._scheduledTimeouts.set(Q, setTimeout(
              () => {
          ...
          "runtime.clearTimeoutEvent": (A) => {
            A >>>= 0;
            const Q = this.mem.getInt32(A + 8, !0);
            clearTimeout(this._scheduledTimeouts.get(Q)), this._scheduledTimeouts.delete(Q);
          },
          ...
          "syscall/js.finalizeRef": (A) => {
            ...
            if (this._goRefCounts[Q]--, this._goRefCounts[Q] === 0) {
              ...
              this._values[Q] = null, this._ids.delete(g), this._idPool.push(Q);
            }
          },
          ...
          "syscall/js.valueCall": (A) => {
            A >>>= 0;
            try {
              const Q = B(A + 8), g = Reflect.get(Q, c(A + 16)), w = k(A + 32), K = Reflect.apply(g, Q, w);
              A = this._inst.exports.getsp() >>> 0, I(A + 56, K), this.mem.setUint8(A + 64, 1);
            } catch (Q) {
              A = this._inst.exports.getsp() >>> 0, I(A + 56, Q), this.mem.setUint8(A + 64, 0);
            }
          },
          ...
          "syscall/js.valueInvoke": (A) => {
            A >>>= 0;
            try {
              const Q = B(A + 8), g = k(A + 16), w = Reflect.apply(Q, void 0, g);
              A = this._inst.exports.getsp() >>> 0, I(A + 40, w), this.mem.setUint8(A + 48, 1);
            } catch (Q) {
              A = this._inst.exports.getsp() >>> 0, I(A + 40, Q), this.mem.setUint8(A + 48, 0);
            }
          },
          ...
          "syscall/js.valueNew": (A) => {
            A >>>= 0;
            try {
              const Q = B(A + 8), g = k(A + 16), w = Reflect.construct(Q, g);
              A = this._inst.exports.getsp() >>> 0, I(A + 40, w), this.mem.setUint8(A + 48, 1);
            } catch (Q) {
              A = this._inst.exports.getsp() >>> 0, I(A + 40, Q), this.mem.setUint8(A + 48, 0);
            }
          ...
          "syscall/js.valueInstanceOf": (A) => {
            A >>>= 0, this.mem.setUint8(A + 24, B(A + 8) instanceof B(A + 16) ? 1 : 0);
          },
          // func copyBytesToGo(dst []byte, src ref) (int, bool)
          "syscall/js.copyBytesToGo": (A) => {
            A >>>= 0;
            const Q = y(A + 8), g = B(A + 32);
    ...
    async run(M) {
      ...
      this._inst = M, this.mem = new DataView(this._inst.exports.mem.buffer), this._values = [
      ...
      let C = 4096;
      const B = (A) => {
        const Q = C, g = D.encode(A + "\0");
        return new Uint8Array(this.mem.buffer, C, g.length).set(g), C += g.length, C % 8 !== 0 && (C += 8 - C % 8), Q;
      }, I = this.argv.length, y = [];
      this.argv.forEach((A) => {
        y.push(B(A));
      }), y.push(0), Object.keys(this.env).sort().forEach((A) => {
      ...
      this._inst.exports.run(I, c), this.exited && this._resolveExitPromise(), await this._exitPromise;
    }
    _resume() {
      if (this.exited)
...
const h = async (E, D) => {
  ...
  }), WebAssembly.instantiateStreaming ? U = await WebAssembly.instantiateStreaming(
    fetch(E),
    D
  ) : U = await (async () => {
    const C = await fetch(E).then(
      (B) => B.arrayBuffer()
    );
...
class p {
  constructor() {
    this.inputLocalWasm = 'http://192.168.1.104:5500/wasm/main.wasm', this.go = new Go(), this.wasm = null, this.importObject = this.go.importObject;
  }
  async initWasm() {
    let D, U, M;
    return await h(this.inputLocalWasm, this.go.importObject).then((C) => {
      D = C.module, U = C.instance, M = U.exports, this.wasm = { inst: U, mod: D, exports: M }, this.go.run(this.wasm.inst);
    }).catch((C) => {
      console.error(C);
    });
  }
